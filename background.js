...
function onRequestHandler(req, msg) {
    if(req.operation=="runEditor") {debugger;
...
function pruebaEnvioRunEditor(){
	chrome.runtime.sendMessage({ operation: 'runEditor'});
}

function onBrowserActionClicked(){
	chrome.tabs.query({active:true,windowType:"normal", currentWindow: true},function(d){
		chrome.scripting.executeScript({
			target: {tabId: d[0].id, allFrames: true},
			func: pruebaEnvioRunEditor,
		});
	});
}

function pruebaEnviodeleteEditor(){
	chrome.runtime.sendMessage({ operation: 'deleteEditor'});
}
...
/*	if(request.operation=="guardarCommonClick"){
  ...
  */  if(request.operation=="currentWebPageStatus"){
        chrome.tabs.query({active:true,windowType:"normal", currentWindow: true}, function (tabId) {debugger;
            chrome.runtime.sendMessage({ operation: 'checkPopup', url:tabId[0].url });
        });
    }
/*
