...
function cargar(){
	...
	//$( "#importexport" ).menu();
	//desactivarTodos();
    chrome.runtime.sendMessage({ operation: 'currentWebPageStatus' });
}

chrome.runtime.onMessage.addListener( function(request, sender, sendResponse) {
    if(request.operation=="checkPopup"){
        checkAugmentation(request.url);console.log(window.location.href);
    }
});

function checkAugmentation(url){
    chrome.storage.local.get(null,function(items) {debugger;//Habra que ver en el elsif que pasa si cambia la URL
...
function hayAumentacionAqui (items, url){
    ...
    if(items.executing!=undefined && items.executing != null) {
        for (var i = 0; i < items.executing.length; i++) {
			if (url.includes(items.executing[i].dominio.host)) {//antes en vez de href estaba host
				hayAumentacion = true;
			}
        }
    }
...
function startEditing (){
    chrome.runtime.sendMessage({operation:"startAugmentation"});
    window.close();
}

function deleteAugmentation(){
    chrome.runtime.sendMessage({operation:"deleteAugmentation"});
    window.close();
}

function guardarEditing(){
	chrome.runtime.sendMessage({operation:"saveAugmentation"});
	window.close();
}
