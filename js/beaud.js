// FASE INICIAL
var div1='<span>Content extraction is being processed. If the corresponding tab does not close in a while, there has been a failure. <ul style="display:none;"><li>See on the opened in which step has happened the error</li></ul></span>';
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse){
    ...
    if(request.operation=="startEditing"){
		...
		chrome.runtime.sendMessage({location:location, operation:"inicio"});
		cargarDesarrollo();
    }
	if(request.operation=="parallelAnswer"){
		paralelizando = false;
		$(".ws_direccion").after("<div class='ws_paralelos'>"+request.html+"</div>");
		$(".ws_direccion").remove();
	}  
	...
	if(request.operation=="terminando"){debugger;
...
/*	if(request.operation=="spiner"){
	...
	if(request.operation=="erroresDetectados"){debugger;
		...
		for(var i = 0; i < items.moment.exec.steps.length; i++){
			...
			for(var j = 0; j < items.moment.exec.steps[i].length; j++){
				if(items.moment.exec.steps[i][j].type == "paralelizando"){
					...
					if(node!=null){
						$(node).after("<div id='wm_insertado"+i+"' class='wm_insertedFinish' style='background-color: #808000; color: #ffffff; padding: 0 3px;font-size: 15px;'>"+div1+"</div>");
					}else{
						$('body').after('<div id="wm_insertado"'+i+' class="wm_insertedFinish" style="background-color: #808000; color: #ffffff; padding: 0 3px;font-size: 15px;">I CANNOT COMPLETE STEP '+i+' AND THE EXECUTION FAILS</div>');
					}
				}
			}
		}
	}else{//Si no se extraen elementos de otras web, para meter mensaje de error
		$('body').after('<div id="wm_insertado0" class="wm_insertedFinish" style="background-color: #808000; color: #ffffff; padding: 0 3px;font-size: 15px;">I CANNOT COMPLETE STEP AND THE EXECUTION FAILS</div>');
	}
}
...
function executeGuardarCopy(data, items){
	...
	if(textNode == null){
		chrome.runtime.sendMessage({operation:"errorDetected", i:data.i, xpath:data.paso.data.xpath, url: location.origin});debugger;
	}try{
...
function executeGuardarCommonClick(data, items){
	...
	if(nodeToClick == null){
		chrome.runtime.sendMessage({operation:"errorDetected", i:data.i, xpath:data.paso.data, url: location.origin});debugger;
	}try{
		nodeToClick.click();
...
function executeGuardarCopyFinalisimo(data, items, i, j){
	...
	if(textNode == null){
		chrome.runtime.sendMessage({operation:"errorDetected", i:i, xpath:data.data.xpath, url: data.location.origin});debugger;
	}
		var textoToSearchFull = textNode.outerText;
...
function executeGuardarCommonClickFinalisimo(data, items, i, j){
	...
	if(nodeToClick == null){
		chrome.runtime.sendMessage({operation:"errorDetected", i:i, xpath:data.data, url: data.location.origin});debugger;
	}
		nodeToClick.click();
...
function executeGuardarHTMLFinalisimo(data, items, i, j){
	...
	chrome.storage.local.set(items,function(){
		chrome.runtime.sendMessage({operation:"finalishedThisSteps", i:i, html:html});
	});
...
function insertarDivParaURL(elem) {
 ...
 //   }else{
        elem.after("<div class='ws_direccion'>INSERT THE URL OF THE WEB PAGE FROM WHICH YOU WANT TO EXTRACT INFORMATION: <br> <input id='ws_helbidea'> <br> <p class='wm_wrongurl'>* THIS IS NOT A VALID URL. PLEASE INSERT A CORRECT ONE.</p> <button class='ws_urlButton'> GO AHEAD!</button> </div>");
 //   }
//	$("#ws_divfterbefore").remove();
    $(".ws_urlButton").click(function(e){debugger;
        ...
        if(is_url(helbidea)){
			...
			chrome.runtime.sendMessage({type:"clickSearchButton", data:helbidea, location:location, operation:"saveStep", paralelizando:paralelizando});
            e.preventDefault();
            e.stopPropagation();
...
function htmlCopy(ev, helbidea, elem, location){
    ...
    $(".ws_urlButton").click(function(e){debugger;
        ...
        $(".ws_direccion").remove();
        $(".ws_urlButton").off("click");
        chrome.runtime.sendMessage({type:"guardarPrimero", copy:copy, helbidea:helbidea, augmentedWebsite:document.URL, dominio:document.domain});
    });
}
								...
								for(var i = 0; i < items.moment.exec.steps.length; i++){
									for (var j = 0; j < items.moment.exec.steps[i].length; j++){
										if(items.moment.exec.steps[i][j].type == "clickSearchButton" && items.moment.exec.steps[i][j].data.includes(location.host)){//items.moment.exec.steps[i][j].data == location.href
											chrome.runtime.sendMessage({operation:"getTabId", i:i, j:j, url:location.host});
											encontradoParalelizacion = true;
										}/*else if(items.moment.exec.steps[i][j].type == "clickSearchButton"){
											chrome.runtime.sendMessage({operation:"getTabId", i:-1, j:-1, url:null});
											encontradoParalelizacion = true;
										}*/
									}
								}
								if(!encontradoParalelizacion){
									chrome.runtime.sendMessage({operation:"getTabId", i:-2, j:-1, url:null});
								}
			}
		}
	});
		...
		for (var j = 0; j < moment.exec.steps[i].length; j++){
			if(moment.exec.steps[i][j].type == "clickSearchButton"){
				if(moment.exec.steps[i][j].data == location.href){
					chrome.runtime.sendMessage({i:i, j:j, location:location, operation:"getTabId"});
				}
			}
		}
	}
...
function cargarAumentacion(exec){
	chrome.runtime.sendMessage({operation:"aumentacionFinal", exec:exec, documento:document, location:location});
}

var paralelizando = false;
function cargarDesarrollo(){
	 $(document).mouseover(function(e){
        var elem = $(e.target);debugger;
    ...
    $(document).click(function(e){debugger;
		...
		if(clicks===1){
			try{
				if(window.getSelection().getRangeAt(0).startOffset == window.getSelection().getRangeAt(0).endOffset){ //SI NO HAY NADA SELECCIONADO
					...
					if(xpath!=".//input[@id='ws_helbidea']"){
						chrome.runtime.sendMessage({type:"guardarCommonClick", data:xpath, location:location, operation:"saveStep"});
					}
				}
			}catch(err){
				var xpath = minimizeXPath(generateXPath(e.target,document,null,{showId:true}),document).join("");debugger;
			...
			timer = setTimeout(function(){debugger;
				$(elem).removeClass("ws_nonSartu");
				elem.css({cursor:""});
				   //     $(document).off("mouseover");
				   //     $(document).off("mouseout");
		...
		//	$("#ws_divfterbefore").remove();
			if(paralelizando == false){
				...
				chrome.runtime.sendMessage({operation:"paralelizando", paralelizando:true, xpath:xpath, location:location});
			}else{
				...
				chrome.runtime.sendMessage({operation:"paralelizando", paralelizando:false, html:html});debugger;
				chrome.runtime.sendMessage({type:"guardarHTML", data:xpathLastStep, location:location, operation:"saveStep"});
			}
	...
	$(document).on('copy', function(e){debugger;
		...
		if(texto!=""){
			...
			chrome.runtime.sendMessage({type:"guardarCopy", data:copy, location:location, operation:"saveStep"});
		}
	//	 e.preventDefault();
    //    e.stopPropagation();
	});
	...
	$(document).on('paste', function(e){
		...
		debugger;
			...
			chrome.runtime.sendMessage({type:"guardarPaste", data:paste, location:location, operation:"saveStep"});
	//		 e.preventDefault();
	//        e.stopPropagation();
		}, 0);
	});
	...
	$(document).on('keypress', function(e){
		...
		chrome.runtime.sendMessage({type:"guardarkeys", data:key, location:location, operation:"saveStep"});
//		 e.preventDefault();
 //       e.stopPropagation();
	});
}